...
public class BankTransactionProducer {
    public static void main(String[] args) {
        ...
        String TOPIC_NAME="bank-balance-updates-v1";

        Properties prodcerConfig = new Properties();
        prodcerConfig.put(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG,BOOTSTRAP_SERVER);
        prodcerConfig.put(ProducerConfig.VALUE_SERIALIZER_CLASS_CONFIG, StringSerializer.class);
        ...
        KafkaProducer<String, CustomerProfile> producer = new KafkaProducer<String, CustomerProfile>(prodcerConfig);
        int i=0;
        while(i<=100){

            CustomerProfile customer=getCustomerProfile();
            ProducerRecord records=new ProducerRecord<String, CustomerProfile>(TOPIC_NAME,customer.getName(),getCustomerProfile());
